---
title: "pelaverse"
description: "A group of packages to gather all Distance Sampling and modelling routine analysis"
# date: "2026-04-30"
image: pelaverse_logo.png
categories: 
  - R
  - package
# layout: single
...
format: 
  html:
    ...
    toc: true
    # toc-location: left
    reference-location: margin  # Place les footnotes dans la marge droite
    citation-location: margin   # Place les citations dans la marge droite
    # page-layout: article
...
__"pelaverse"__ is a group on La Rochelle university gitlab that gather a bunch of packages created by [pelagis](https://www.observatoire-pelagis.cnrs.fr/). Its name is a reference to [__tidyverse__](https://www.tidyverse.org/), _"a collection of packages"_ helping to manipulate and visualize data. In the same spirit as tidyverse, pelaverse aims at gathering R packages used/created by pelagis for the use of pelagis and collaborators.
...
![Aerial transect of marine megafauna. Plane flying over the ocean (left). Observer spotting animals on the water (middle). Sightings recorded in the field computer (right)](img/plane_transect.png)
...
### 2. Data used in this example
 ...
 For this example, we will use the data of the pelaCDS package __`SPEE_data`__. This data contains a list of multiple data, we will keep only `effort_raw` and `obs_with_sp` corresponding to the effort transect of SPEE campaign and associated observations for 2 groups : 
...
By running the previous lines of code, you will create a repository __my_cds_analysis__ with this tree : 

<!-- ![tree](tree.png){#fig-tree width='100px'} -->

<img src="img/tree.png" width="50%" title="Titre de la figure">

Data have to be in __data__ sub-directory (In our example, effort and observation are already in pelaCDS package).
